 ...
 */
class Sie_Modules extends Model
{
    protected $table = "sie_modules";
...
//$forge->addKey('profile');
    ...
    {
        $result = $this->where('module', $module)->first();
        if (is_array($result)) {
            return $result;
        } else {
            return false;
    ...
    {
        $result = $this->select("`{$this->primaryKey}` AS `value`, CONCAT(`name`, ' - ', `reference`) AS `label`")->orderBy('name', 'DESC')->findAll();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
