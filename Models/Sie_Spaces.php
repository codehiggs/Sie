...
/**
 * Ej: $model = model('App\Modules\Sie\Models\Sie_Spaces');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Spaces extends Model
{
    protected $table = "sie_spaces";
    protected $primaryKey = "space";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'space' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'headquarter' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'type' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'block' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'name' => ['type' => 'TINYTEXT', 'null' => FALSE],
                'description' => ['type' => 'TEXT', 'null' => FALSE],
                'area' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
                'static_capacity' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
                'dynamic_capacity' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
                'tv' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N',],
                'split_wall' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N',],
                'split_ceiling' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N',],
                'videobeam' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N',],
                'sound' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N',],
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("space", "%{$search}%")
            ->orLike("headquarters", "%{$search}%")
            ->orLike("type", "%{$search}%")
            ->orLike("block", "%{$search}%")
            ->orLike("name", "%{$search}%")
            ->orLike("description", "%{$search}%")
            ->orLike("area", "%{$search}%")
            ->orLike("static_capacity", "%{$search}%")
            ->orLike("dynamic_capacity", "%{$search}%")
            ->orLike("tv", "%{$search}%")
            ->orLike("split_wall", "%{$search}%")
            ->orLike("split_ceiling", "%{$search}%")
            ->orLike("videobeam", "%{$search}%")
            ->orLike("sound", "%{$search}%")
     ...
     * @return array|false
     */
    public function get_Space($space): false|array
    {
        $key = $this->get_CacheKey("space-{$space}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $space)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
