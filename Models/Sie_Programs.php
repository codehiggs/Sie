 ...
 */
class Sie_Programs extends Model
{
    protected $table = "sie_programs";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'program' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'reference' => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => FALSE],
                'name' => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => FALSE],
                'acronym' => ['type' => 'VARCHAR', 'constraint' => 32, 'null' => FALSE],
                'snies' => ['type' => 'VARCHAR', 'constraint' => 32, 'null' => FALSE],
                'ies' => ['type' => 'VARCHAR', 'constraint' => 32, 'null' => FALSE],
                'ies_parent' => ['type' => 'VARCHAR', 'constraint' => 32, 'null' => FALSE],
                'credits' => ['type' => 'INT', 'constraint' => 5, 'null' => FALSE],
                'resolution' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'resolution_date' => ['type' => 'DATE', 'null' => FALSE],
                'resolution_execution' => ['type' => 'DATE', 'null' => FALSE],
                'evaluation' => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => FALSE],
                'academic_level' => ['type' => 'VARCHAR', 'constraint' => 128, 'null' => FALSE],
                'modality' => ['type' => 'VARCHAR', 'constraint' => 64, 'null' => FALSE],
                'education_level' => ['type' => 'VARCHAR', 'constraint' => 64, 'null' => FALSE],
                'awarded_title' => ['type' => 'VARCHAR', 'constraint' => 255, 'null' => FALSE],
                'groups' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N',],
                'preregistration' => ['type' => 'ENUM', 'constraint' => ['Y', 'N'], 'default' => 'N',],
...
//$forge->addKey('profile');
    ...
    public function get_Program($program): false|array
    {
        $result = $this->where('program', $program)->first();
        if (is_array($result)) {
            return $result;
        } else {
            return false;
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ...
            ->orLike("resolution", "%{$search}%")
            ->orLike("resolution_date", "%{$search}%")
            ->orLike("evaluation", "%{$search}%")
            ->orLike("groups", "%{$search}%")
            ->orLike("preregistration", "%{$search}%")
    ...
    /**
     * Retorna solo los programas que tienen la opción de preinscripción activa.
     * @return array|false
     */
    public function get_SelectPreregistration()
    {
        $result = $this
            ->select("`{$this->primaryKey}` AS `value`,`name` AS `label`")
            ->where('preregistration', 'Y')
            ->findAll();
        if (is_array($result)) {
            return ($result);
        } else {
