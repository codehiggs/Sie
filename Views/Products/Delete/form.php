...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2024-08-08 10:22:17
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Products\Delete\form.php]
...
$r = $model->get_Product($oid);
$name = urldecode($r["name"]);
$message = sprintf(lang("Products.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Products.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Products.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
