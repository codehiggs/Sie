 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Modules\Delete\form.php]
...
$r = $model->where('module', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Modules.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Modules.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Modules.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
