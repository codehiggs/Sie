 ...
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Courses\Delete\form.php]
...
$r = $model->where('course', $oid)->first();
$name = urldecode($r["name"]);
$message = sprintf(lang("Courses.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Courses.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Courses.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
