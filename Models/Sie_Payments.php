...
class Sie_Payments extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'record_type' => ['type' => 'VARCHAR', 'constraint' => 3, 'null' => FALSE],
                'agreement' => ['type' => 'VARCHAR', 'constraint' => 4, 'null' => FALSE],
                'id_number' => ['type' => 'VARCHAR', 'constraint' => 10, 'null' => FALSE],
                'ticket' => ['type' => 'VARCHAR', 'constraint' => 6, 'null' => FALSE],
                'value' => ['type' => 'DECIMAL', 'null' => FALSE],
                'description' => ['type' => 'TEXT', 'null' => TRUE],
                'payment_origin' => ['type' => 'VARCHAR', 'constraint' => 2, 'null' => FALSE],
                'payment_methods' => ['type' => 'VARCHAR', 'constraint' => 2, 'null' => FALSE],
                'operation_number' => ['type' => 'VARCHAR', 'constraint' => 6, 'null' => FALSE],
                'authorization' => ['type' => 'VARCHAR', 'constraint' => 6, 'null' => FALSE],
                'financial_entity' => ['type' => 'VARCHAR', 'constraint' => 6, 'null' => FALSE],
                'branch' => ['type' => 'VARCHAR', 'constraint' => 4, 'null' => FALSE],
                'sequence' => ['type' => 'VARCHAR', 'constraint' => 7, 'null' => FALSE],
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("payment", "%{$search}%")
            ->orLike("record_type", "%{$search}%")
            ->orLike("agreement", "%{$search}%")
            ->orLike("id_number", "%{$search}%")
            ->orLike("ticket", "%{$search}%")
            ->orLike("value", "%{$search}%")
            ->orLike("payment_origin", "%{$search}%")
            ->orLike("payment_methods", "%{$search}%")
            ->orLike("operation_number", "%{$search}%")
            ->orLike("authorization", "%{$search}%")
            ->orLike("financial_entity", "%{$search}%")
            ->orLike("branch", "%{$search}%")
            ->orLike("sequence", "%{$search}%")
            ->groupEnd()
     ...
     * @return array|false
     */
    public function get_Payment($payment): false|array
    {
        $key = $this->get_CacheKey("payment-{$payment}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $payment)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
