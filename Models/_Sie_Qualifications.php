...
/**
 * Ej: $model = model('App\Modules\Sie\Models\Sie_Qualifications');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Qualifications extends Model
{
    protected $table = "sie_qualifications";
    protected $primaryKey = "qualification";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'qualification' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'teacher' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'score' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
                'weighting' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 0.00, 'null' => FALSE],
                'update_at' => ['type' => 'DATETIME', 'null' => FALSE],
                'delete_at' => ['type' => 'DATETIME', 'null' => FALSE],
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("qualification", "%{$search}%")
            ->orLike("teacher", "%{$search}%")
            ->orLike("score", "%{$search}%")
            ->orLike("weighting", "%{$search}%")
            ->orLike("author", "%{$search}%")
            ->orLike("update_at", "%{$search}%")
            ->orLike("delete_at", "%{$search}%")
            ->groupEnd()
     ...
     * @return array|false
     */
    public function get_Qualification($qualification): false|array
    {
        $key = $this->get_CacheKey("qualification-{$qualification}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $qualification)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
