...
//[Query]---------------------------------------------------------------------------------------------------------------
$list = $mregistrations->get_List($limit, $offset, $search);
$recordsTotal = $mregistrations->get_Total($search);
//$sql=$mregistrations->getLastQuery()->getQuery();
//[Asignations]---------------------------------------------------------------------------------------------------------
$data = array();
$component = '/sie/registrations';
foreach ($list as $item) {
    if (!empty($item["agreement"])) {
        $agreement = $magreements->get_Agreement($item["agreement"]);
    ...
    $options = $bootstrap::get_BtnGroup('options', array('content' => array($lviewer, $leditor, $lbilling, $ldeleter, $lnotifier, $lscheduler)));
    ...
    $row["full_name"] = $item["first_name"] . " " . $item["second_name"] . " " . $item["first_surname"] . " " . $item["second_surname"] . "<br>" . $status;
    ...
    $row["place_of_residence"] = $item["place_of_residence"];
    ...
    $row["place_of_birth"] = $item["place_of_birth"];
    ...
    $row["status"] = $status;
    $row["agreement"] = !empty($item["agreement"]) ? $agreement["name"] : "";
    $row["created_at"] = $item["created_at"];
    if (!empty($item["created_at"])) {
        $fecha = new DateTime($item["created_at"]);
        $row["date"] = $fecha->format('Y-m-d');
    } else {
        $row["date"] = "";
    }
    $row["options"] = $options;
