...
$registration = $mregistrations->get_Registration($order['user']);
...
$text = is_array($program) ? mb_convert_encoding("$ " . $numbers->get_NumberFormat($capital, 0), 'ISO-8859-1', 'UTF-8') : "";
$pdf->SetXY($xvalue, $y);
$pdf->Cell(0, 1, $text, 0, 1, 'L');
...
$pdf->SetXY($xlabel, $y);
$pdf->Cell(108, 1, mb_convert_encoding("", 'ISO-8859-1', 'UTF-8'), 0, 1, 'R');
$text = is_array($program) ? mb_convert_encoding($expiration, 'ISO-8859-1', 'UTF-8') : "";
$pdf->SetXY($xvalue, $y);
$pdf->Cell(0, 1, $text, 0, 1, 'L');
