...
/**
 * Ej: $model = model('App\Modules\Sie\Models\Sie_Orders_Items');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Orders_Items extends Model
{
    protected $table = "sie_orders_items";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'item' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'order' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'type' => ['type' => 'ENUM', 'constraint' => ['UNIT_PRICE', 'SUBTOTAL', 'PERCENTAGE_DISCOUNT', 'FIXED_DISCOUNT', 'GENERAL_TAX', 'SPECIFIC_TAX', 'SURCHARGE', 'ITEM_TOTAL'], 'default' => 'UNIT_PRICE',],
                'value' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
                'amount' => ['type' => 'DOUBLE', 'constraint' => '10,2', 'default' => 1.00, 'null' => FALSE],
                'description' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
                'percentage' => ['type' => 'DECIMAL', 'null' => FALSE],
     ...
     * @return array|false
     */
    public function get_Item($item): false|array
    {
        $key = $this->get_CacheKey("item-{$item}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $item)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
