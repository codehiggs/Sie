...
/**
 * Ej: $model = model('App\Modules\Sie\Models\Sie_Agreements');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Agreements extends Model
{
    protected $table = "sie_agreements";
    protected $primaryKey = "agreement";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'agreement' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'name' => ['type' => 'TINYTEXT', 'null' => FALSE],
                'description' => ['type' => 'MEDIUMTEXT', 'null' => FALSE],
                'country' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'region' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'city' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'institution' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'headquarters' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
     ...
     * @return array|false
     */
    public function get_Agreement($agreement): false|array
    {
        $key = $this->get_CacheKey("agreement-{$agreement}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $agreement)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
