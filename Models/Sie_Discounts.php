...
/**
 * @syntax: $mdiscounts = model("App\Modules\Sie\Models\Sie_Discounts");
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Discounts extends Model
{
    protected $table = "sie_discounts";
    protected $primaryKey = "discount";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'discount' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'reference' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'name' => ['type' => 'TINYTEXT', 'null' => FALSE],
                'description' => ['type' => 'TEXT', 'null' => FALSE],
                'value' => ['type' => 'FLOAT', 'null' => FALSE],
                'type' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'character' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("discount", "%{$search}%")
            ->orLike("reference", "%{$search}%")
            ->orLike("name", "%{$search}%")
            ->orLike("description", "%{$search}%")
            ->orLike("value", "%{$search}%")
            ->orLike("type", "%{$search}%")
            ->orLike("character", "%{$search}%")
     ...
     * @return array|false
     */
    public function get_Discount($discount): false|array
    {
        $key = $this->get_CacheKey("discount-{$discount}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $discount)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
