 ...
 */
class Sie_Products extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'product' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                ...
                'value' => ['type' => 'FLOAT', 'unsigned' => TRUE],
                'taxes' => ['type' => 'FLOAT', 'unsigned' => TRUE],
...
//$forge->addKey('profile');
    ...
    {
        $key = $this->get_CacheKey("product-{$product}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $product)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
