 ...
 */
class Sie_Versions extends Model
{
    protected $table = "sie_versions";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'grid' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'reference' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
...
//$forge->addKey('profile');
    ...
    /**
     * Obtiene un registro de la base de datos utilizando el identificador único del registro.
     * @return void
     */
    public function setAllInactive($grid)
    {
        if (!empty($grid)) {
     ...
     * @return array|false
     */
    public function get_Version($version): false|array
    {
        $key = $this->get_CacheKey("version-{$version}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $version)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
    ...
    /**
     * Retorna la version activa de un grid
     * @param $grid
     * @return false|array
     */
    public function get_Active($grid): false|array
    {
        $row = $this
            ->where('grid', $grid)
    ...
    {
        $key = $this->get_CacheKey("version-{$id}");
        cache()->delete($key);
    }
