...
/**
 * Ej: $model = model('App\Modules\Sie\Models\Sie_Institutions');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Institutions extends Model
{
    protected $table = "sie_institutions";
    protected $primaryKey = "institution";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'parent' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => TRUE],
                'name' => ['type' => 'TINYTEXT', 'null' => FALSE],
                'description' => ['type' => 'MEDIUMTEXT', 'null' => TRUE],
                'address' => ['type' => 'TINYTEXT', 'null' => TRUE],
     ...
     * @return array|false
     */
    public function get_Institution($institution): false|array
    {
        $key = $this->get_CacheKey("institution-{$institution}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $institution)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
