...
/**
 * █ ---------------------------------------------------------------------------------------------------------------------
 * █ ░FRAMEWORK                                  2024-08-06 16:36:01
 * █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Sie\Views\Enrolleds\Delete\form.php]
...
$r = $model->where('enrolled', $oid)->first();
$name = urldecode($r["enrolled"]);
$message = sprintf(lang("Enrolleds.delete-message"), $name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
    "class" => "card-info",
    "icon" => ICON_WARNING,
    "title" => sprintf(lang("Enrolleds.delete-title"), $name),
    "text-class" => "text-center",
    "text" => sprintf(lang("Enrolleds.delete-message"), $name),
    "content" => $f,
    "header-back" => $back
));
echo($card);
