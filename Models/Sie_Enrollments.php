...
/**
 * Ej: $model = model('App\Modules\Sie\Models\Sie_Enrollments');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Sie_Enrollments extends Model
{
    protected $table = "sie_enrollments";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'student' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                ...
                'observation' => ['type' => 'TEXT', 'null' => FALSE],
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("enrollment", "%{$search}%")
            ->orLike("student", "%{$search}%")
            ->orLike("program", "%{$search}%")
            ->orLike("grid", "%{$search}%")
            ->orLike("version", "%{$search}%")
            ->orLike("observation", "%{$search}%")
            ->orLike("date", "%{$search}%")
            ->orLike("time", "%{$search}%")
     ...
     * @return array|false
     */
    public function get_Enrollment($enrollment): false|array
    {
        $key = $this->get_CacheKey("enrollment-{$enrollment}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $enrollment)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
