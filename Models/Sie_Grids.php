 ...
 */
class Sie_Grids extends Model
{
    protected $table = "sie_grids";
...
//$forge->addKey('profile');
     ...
     */
    public function get_SelectData($program): array|false
    {
        $result = $this
            ->select("`{$this->primaryKey}` AS `value`,`name` AS `label`")
            ->where("program", $program)->findAll();
        if (is_array($result)) {
            return ($result);
        } else {
            return (false);
     ...
     * @return array|false
     */
    public function get_Grid($grid): false|array
    {
        $key = $this->get_CacheKey("grid-{$grid}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $grid)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
    ...
    public function get_GridByProgram($program): false|array
    {
        $row = $this->where("program", $program)->first();
        if (is_array($row)) {
            $value = $row;
        } else {
            $value = false;
